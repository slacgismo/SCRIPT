...
# List of Counties in Analysis - For Base Case, use all Counties
# county_list = pd.read_csv(
#     DIRECTORY + "/county_list/ev_distribution.csv")['County'].tolist()
...
WEEKDAY_SLAC_DATA_PATH = DIRECTORY + "/inputs/"
# WEEKEND_SLAC_DATA_PATH = "S:\E3 Projects\CEC SLAC EV\Data processing\Load Shapes From SLAC\SLAC Final Load Profiles July 2020\Weekend SLAC Loads"

weekday_load_list = os.listdir(WEEKDAY_SLAC_DATA_PATH)
print(weekday_load_list)
# weekend_load_list = os.listdir(WEEKEND_SLAC_DATA_PATH)

# Scenarios that are to be run

scenario_dictionary = { "Scenario 1": "BaseCase", "Scenario 2": "LowAdoption", "Scenario 3": 'HighAdoption',
                       'Scenario 4': 'WorkPublic', 'Scenario 5': 'FastPublic', 'Scenario 6': "Work",
                       'Scenario 7': 'Equity'}

# Driver Counts Path

driver_counts_path = DIRECTORY + "/driver_counts"

# Adoption File - This file must correspond to the scenario selected.

e3_adoption_path = DIRECTORY + "/adoption_files/vehicle_adoption_base_case.xlsx"
...
for Scenario in scenario_dictionary.values():
    print('scenario')
    print(Scenario)
    for Management in ["uncontrolled", "e19controlled"]:
        print("management")
        print(Management)
        for county in county_list:
            print(county)
            for SLAC_Year in ['2025', '2030']:
                ...
                if Scenario in ['WorkPublic', 'FastPublic', "Work", 'Equity']:
                    ...
                    weekday_file_name = "{}_{}_weekday_{}_county_{}_load.csv".format(Scenario, SLAC_Year, county,
                                                                                              Management)
                    # weekend_file_name = "{}_{}_weekend_{}_county_{}_load.csv".format(Scenario, SLAC_Year, county,
                                                                                            #   Management)

                driver_count = float(driver_count_df.loc[driver_count_df['County'] == county]['Num Drivers'])
                e3_adoption_spreadsheet_df = \
                pd.read_excel(open(e3_adoption_path, 'rb'), sheet_name=county).set_index(['year'])[
                    ' BEV_population']
                ...
                # day_type = pd.read_csv(os.path.join(DIRECTORY,'day_type.csv'))
                ...
                for field_name in col_names:
                    ...
                    if SLAC_Year == "2025":
                        for year in range(2019, 2031):
                            output_dictionary[field_name][year] = per_vehicle_results[SLAC_Year] * e3_adoption_spreadsheet_df.loc[year]/1000
                    elif SLAC_Year == "2030":
                        year = 2030
                        output_dictionary[field_name][year] = per_vehicle_results[SLAC_Year] * e3_adoption_spreadsheet_df.loc[year]/1000
                    ...
                    if SLAC_Year == "2030":
                        ...
                        stock_rollover_output.to_csv(os.path.join(
                            DIRECTORY, "outputs",  "{}_{}_{}_{}_load.csv".format(
