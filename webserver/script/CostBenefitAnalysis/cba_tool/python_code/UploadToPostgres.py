...
class UploadToPostgres():
    def __init__(
        ...
        #this will be what the rest api calls to run the CBA with 
        #the config name for the load forecast tables will be a parameter (demo2 for testing)
        self.cur = self.conn.cursor()
        query = """SELECT * FROM script_algorithm_ev_load_forecast WHERE config='demo2'"""

        #assumption is to turn this table into a csv inserted in EV_Load/load_profiles
        outputquery = "COPY ({0}) TO STDOUT WITH CSV HEADER".format(query)
        parent_path = Path(__file__).parent.absolute()
        path = parent_path.parent

        with open(str(path) + '/EV_Loads/load_profiles/demo2.csv', 'w') as f:
            self.cur.copy_expert(outputquery, f)

        #once set up for the load profiles or how the inputs should exist (my assumption)
        #then run ...
        #model = model_class.ModelInstance()
        
        self.conn.close()

UploadToPostgres(load_profile = 'demo2')

#################################################################################################################################
#### BELOW IS COMMENTED OUT CODE - THIS SHOULD WORK ACCORDINGLY (some minor changes for hard code names) ONCE CBA TOOL WORKS ####
#################################################################################################################################


    #     with open(settings.BASE_DIR[:-3] + 'script/CostBenefitAnalysis/postgres_info.json') as json_file:
    #         postgres_info = json.load(json_file)
    ...
    #     with open(settings.BASE_DIR[:-3] + 'script/CostBenefitAnalysis/results/AllDCFC_loadprofile.csv') as csv_file:
    #         csv_reader = csv.reader(csv_file, delimiter=',')
    ...
    #     with open(settings.BASE_DIR[:-3] + 'script/CostBenefitAnalysis/results/AllPublicL2_loadprofile.csv') as csv_file:
    #         csv_reader = csv.reader(csv_file, delimiter=',')
    ...
    #     with open(settings.BASE_DIR[:-3] + 'script/CostBenefitAnalysis/results/AllResidential_loadprofile.csv') as csv_file:
    #         csv_reader = csv.reader(csv_file, delimiter=',')
    ...
    #     with open(settings.BASE_DIR[:-3] + 'script/CostBenefitAnalysis/results/AllWorkplace_loadprofile.csv') as csv_file:
    #         csv_reader = csv.reader(csv_file, delimiter=',')
