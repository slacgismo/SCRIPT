...
class UploadToPostgres():
    ...
    def run(self, baseline_profiles, controlled_profiles):
        ...
        cur.execute("INSERT INTO " + self.table_name + \
            ...
            (
                int(county_id), float(self.rate_energy_peak), float(self.rate_energy_partpeak), float(self.rate_energy_offpeak),
                float(self.rate_demand_peak), float(self.rate_demand_partpeak), float(self.rate_demand_overall), 
